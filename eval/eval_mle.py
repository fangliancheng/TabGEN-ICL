...
if __name__ == '__main__':
    ...
    if not args.path:
        train_path = f'synthetic/{dataname}/{model}.csv' #default: synthetic train data path
    else:
    ...
    # if a column is ' null', drop the sample
    #train = train[~pd.DataFrame(train).isin([' ?']).any(axis=1)] #temporary solution
    #train = train[~pd.DataFrame(train).isin([' 170 Mont.']).any(axis=1)] #temporary solution
    ...
    if not args.exp_name:
        ...
        save_path = f'eval/mle/{dataname}/{args.exp_name}.json'
